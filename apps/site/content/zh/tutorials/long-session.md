...
当前 omdsh 不再写入私有的 `omdsh/tools-selected` 事件，因此新建会话可以由未经扩展的 DSH 持久化读取器加载。最初由更早 omdsh 版本创建的会话——本 fork 截至 0.10.0，以及上游 v0.5.0 至 v0.11.0——仍可能包含该事件：当前 omdsh 可以识别并恢复这些会话，但未经扩展的 DSH 读取器会拒绝这类日志。会话文件可能经过压缩并带有完整性校验，请勿手工修改；在提供显式迁移工具前，请继续使用 omdsh 打开这些旧会话。
