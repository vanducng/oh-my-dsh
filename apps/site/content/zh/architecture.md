...
[`apps/omdsh/config/cordis.yml`](https://github.com/vanducng/oh-my-dsh/blob/main/apps/omdsh/config/cordis.yml) 是 `@vanducng/oh-my-dsh` 产品 bundle，叠在空的 `$OMDSH_HOME/profiles/omdsh` 根上，其中组合了：
...
Skills 与 MCP 的部署细节见 [`skills-and-mcp.md`](skills-and-mcp.md)。`omdsh plugin add` 装入的用户 bundle、Profile 的 `cordis.patch.yml`、可选的 `$OMDSH_HOME/cordis.patch.yml`，以及本 fork 的 `$OMDSH_HOME/omdsh/plugins.yml` 与 `$OMDSH_HOME/omdsh/cordis.patch.yml` 会在启动时叠加在这棵 composition 之上；`omdsh --dump-config` 打印结果。[`examples/hello`](https://github.com/vanducng/oh-my-dsh/tree/main/examples/hello) 是可安装的编写样例。见 [`plugins.md`](plugins.md)。
...
- `pnpm smoke:tui` 把该 PTY 送进 `@xterm/headless`，并在 80x30 单元格网格上断言启动/状态、`/agent`、`@` 文件弹出层和 Ctrl+G。第二次启动会加载一份公共 `vanducng/dotfiles` dsh 主目录的脱敏副本（不提交到本仓库），检查 `dsh-observe` 包含项，并确认 `grok-4.6` 能进入页脚且 `plugins.yml` 仍处于挂载状态。
- 依赖边界检查要求使用已发布的 npm 软件包、保持参考子模块干净，并禁止指向 `refs/` 的链接或别名。

改动所需执行的具体命令由 [`AGENTS.md`](https://github.com/vanducng/oh-my-dsh/blob/main/AGENTS.md) 规定。
